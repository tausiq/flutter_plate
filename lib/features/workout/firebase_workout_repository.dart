...
class FirebaseWorkoutsRepository implements WorkoutRepository {
  ...
  Future<void> deleteWorkout(Workout item) async {
    return workoutCollection.doc(item.id).delete();
  }
  ...
  Future<void> updateWorkout(Workout item) async {
    return await workoutCollection.doc(item.id).update(item.toEntity().toDocument());
  }

  @override
  Future<Workout> getWorkout(String id) async {
    return Workout.fromEntity(
        WorkoutEntity.fromSnapshot(await workoutCollection.doc(id).get()));
  }
  ...
  Stream<List<Workout>> filteredWorkouts(
    ...
    return workoutCollection
        ...
        .where('dateTime', isLessThanOrEqualTo: toDate.toUtc().millisecondsSinceEpoch)
        .snapshots()
        .map((snapshot) {
      return snapshot.docs
          .map((doc) => Workout.fromEntity(WorkoutEntity.fromSnapshot(doc)))
