...
import 'package:flutter_plate/features/app/model/pojo/AppContent.dart';
...
class DBAppStoreRepository {
  ...
  Future<void> saveOrUpdateFeatureApp(AppContent appContent) async {
    ...
    int count = await _database.rawUpdate(
        'UPDATE AppContent SET _order = ?, isFeatureApp = ? , trackName = ? , description = ? , meta = ?  WHERE trackId = ?',
        [
          appContent.order,
          appContent.isFeatureApp,
    ...
    if (0 == count) {
      await _database.rawInsert(
          'INSERT INTO AppContent (trackId, _order, isFeatureApp, trackName, description, meta) VALUES (?, ?, ?, ?, ?, ?)',
          [
            appContent.trackId,
            appContent.order,
            appContent.isFeatureApp,
  ...
  Future<void> saveOrUpdateTopFreeApp(AppContent appContent) async {
    String meta = jsonEncode(appContent.toJson());
    int count = await _database.rawUpdate(
        'UPDATE AppContent SET _order = ?, isFreeApp = ? , trackName = ? , description = ? , meta = ?  WHERE trackId = ?',
        [
          appContent.order,
          appContent.isFeatureApp,
          appContent.trackName,
    ...
    if (0 == count) {
      await _database.rawInsert(
          'INSERT INTO AppContent (trackId, _order, isFreeApp, trackName, description, meta) VALUES (?, ?, ?, ?, ?, ?)',
          [
            appContent.trackId,
            appContent.order,
            appContent.isFreeApp,
            appContent.trackName,
            appContent.description,
            meta
          ]);
  ...
  Future<void> saveOrUpdateDetailApp(AppContent appContent) async {
    ...
    await _database.rawUpdate(
        'UPDATE AppContent SET meta = ?  WHERE trackId = ?', [meta, appContent.trackId]);
  }
  ...
  Future<List<AppContent>> loadFeaturesApp(String searchKey) async {
    ...
    if (searchKey.isNotEmpty) {
      searchSql =
          "AND (`trackName` LIKE '%$searchKey%' OR `description` LIKE '%$searchKey%')";
    }

    List<Map> list = await _database.rawQuery(
        "SELECT  _order, meta FROM AppContent WHERE `isFeatureApp` = 1 $searchSql ORDER BY `_order` ASC");
  ...
  Future<List<AppContent>> loadTopFreeApp(String searchKey) async {
    ...
    List<Map> list = await _database.rawQuery(
        "SELECT  _order, meta FROM AppContent WHERE `isFreeApp` = 1 $searchSql ORDER BY `_order` ASC");
    List<AppContent> applist = [];
  ...
  Future<AppContent> loadAppDetail(num trackId) async {
    List<Map> list = await _database
        .rawQuery("SELECT meta from AppContent WHERE trackId = $trackId LIMIT 1");
    if (list.length > 0) {
